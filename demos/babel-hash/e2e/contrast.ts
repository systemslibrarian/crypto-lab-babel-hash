 ...
 *    files the node under `incomplete`. Both themes here paint `body` with a
 *    radial accent wash over a linear ramp, and every `.panel` in dark theme
 *    carries a `--panel-bg` linear gradient of its own, so most text on this
 *    page composites onto something axe will not measure.
 ...
 *    `opacity: .85` and the reduced-motion block collapses animations to
 *    0.01ms, which preserves end states rather than cancelling them.
 *
 ...
 * rather than against white. A gradient is sampled at the text's own position.
 *
 ...
 *
 * Three further rules, each of which otherwise makes the helper report a ratio
 * nothing on screen has:
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. This lab prints
 *    64-character hex digests and forged MACs in `.digest-block`, and the
 *    comparison tables become `overflow-x: auto` scrollers below 820px, so at
 *    380px a good deal of that content is clipped away. Content scrolled past
 *    the client box is not dimmed or partly drawn — it is absent from the
 *    frame, and asking what colour it sits on has no answer. Its rect is still
 *    to the right of every ancestor's box, so the ancestor walk would find
 *    nothing behind it and fall through to white, inventing a failure. Skip it,
 *    and rely on the wider viewport where the same element is measured for real.
 *
 ...
 *    draws no SVG figures — the avalanche distribution is a row of `div` bars,
 *    not a chart element — but the rule is kept because it is cheap and silent
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * this lab's `--accent` into its surfaces with `color-mix()` in `srgb`, and
     * the hero's "why it matters" card is `color-mix(in oklab, var(--accent) 6%,
     * transparent)`. Chromium reports both to `getComputedStyle` unchanged
     * rather than converting them to sRGB. A
     ...
     *
     * With "compare all three" on, this page renders SIX 256-cell bit grids —
     * 1,536 elements — and every one of them walks the same handful of
     * ancestors. Without
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // Kept as a general guard: `content-visibility: hidden` and similar keep
      // the last laid-out geometry for a subtree, so the `display`/rect tests
      // above all pass for text that paints nothing. This lab hides its
      // inactive tab panels with `display: none`, which the test above already
      // catches, so this line is silent here.
      ...
      // failure for text that is not on screen. The same rule covers the
      // `.sr-only` live-region announcer, which is clipped to a 1px box.
     ...
     * that covers the bit-grid legend swatches and the distribution chart's
     * bars, both marked `aria-hidden` because the numbers beside them carry the
     ...
     *
     * This is not hypothetical elsewhere in the fleet: interpolating a string
     * ARRAY into a template literal (`<g>${dots}</g>`) makes JS join it with
     * commas, leaving a run of stray "," text nodes inside the group, invisible
     * on screen but measurable by a naive walk. This lab has no SVG figures, so
     * the rule is inert here and kept only so a future one is covered.
     */
